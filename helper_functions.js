...
async function fetchAllListings(page, retries = 3) {
  try {
    const response = await axios.get(
      `https://api.printify.com/v1/shops/${process.env.PRINTIFY_SHOP_ID}/products.json/?page=${page}`,
      {
        headers: {
    ...
    if (retries > 0) {
      console.log(
        `Retrying fetchAllListings for page ${page}, (${retries} attempts left)...`
      );
...
async function updateCanadaSkus(listing, retries = 3) {
  ...
  try {
    await axios.put(
      `https://api.printify.com/v1/shops/${process.env.PRINTIFY_SHOP_ID}/products/${listing.listing_id}.json`,
      putObject,
      {
        headers: {
          Authorization: `Bearer ${process.env.PRINTIFY_API_KEY}`,
    ...
    if (retries > 0) {
      console.log(
        `updateCanadaSkus attempt failed. ${retries} retries left. Retrying...`
      );
...
async function createNewCountryListing(newProductTemplate, retries = 3) {
  try {
    ...
    if (retries > 0) {
      console.log(
        `createNewCountryListing attempt failed. ${retries} retries left. Retrying...`
      );
